...
document.getElementById("front_end").addEventListener("click", () => {
  let frontEndEgnineers = developers.filter((dev) => dev.dept === "front_end");
  paintDevelopers(frontEndEgnineers);
});
document.getElementById("back_end").addEventListener("click", () => {
  let backEndEgnineers = developers.filter((dev) => dev.dept === "backend");
  paintDevelopers(backEndEgnineers);
});
